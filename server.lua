...
QBCore.Functions.CreateCallback('epic-djscript:server:syncLocations', function(source, cb) cb(Locations) end)
...
RegisterNetEvent('epic-djscript:server:AddLocation', function(data)
	DestroyAll()
	...
	TriggerClientEvent("epic-djscript:client:syncLocations", -1, Locations)
end)

RegisterNetEvent("epic-djscript:server:syncLocations", function() TriggerClientEvent("jim-jobgarage:client:syncLocations", -1, Locations) end)

RegisterNetEvent('epic-djscript:server:playMusic', function(song, zoneNum)
	...
	TriggerClientEvent('epic-djscript:client:playMusic', src, { zoneNum = zoneNum })
end)

RegisterNetEvent('epic-djscript:server:stopMusic', function(data)
    ...
    TriggerClientEvent('epic-djscript:client:playMusic', src, { zoneNum = data.zoneNum })
end)

RegisterNetEvent("epic-djscript:server:PauseResume", function(data)
    local src, Booth = source, Locations[data.zoneNum]
	local zoneLabel = ""..(Booth.gang or Booth.job)..data.zoneNum
...
RegisterNetEvent('epic-djscript:server:changeVolume', function(volume, zoneNum)
...
QBCore.Functions.CreateCallback('epic-djscript:songInfo', function(source, cb) cb(previousSongs) end)

AddEventHandler('onResourceStop', function(r) if r ~= GetCurrentResourceName() then return end DestroyAll() end)

local function CheckVersion()
	PerformHttpRequest('https://raw.githubusercontent.com/jimathy/epic-djscript/master/version.txt', function(err, newestVersion, headers)
