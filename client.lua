	...
	for i = 1, #Locations do
		if Locations[i].enableBooth then
			...
			exports['qb-target']:AddCircleZone("Booth"..i, Locations[i].coords, 0.6, {name="Booth"..i, debugPoly=Config.Debug, useZ=true, },
				{ options = { { event = "epic-djscript:client:playMusic", icon = "fab fa-youtube", label = Loc[Config.Lan].target["dj_booth"], job = RequireJob, gang = RequireGang, zoneNum = i, coords = Locations[i].coords }, }, distance = 2.0 })
...
local function syncLocations()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('epic-djscript:server:syncLocations', function(cb) p:resolve(cb) end)
...
RegisterNetEvent("epic-djscript:client:syncLocations", function(newLocations) Locations = newLocations makeTargets() end)

RegisterNetEvent("epic-djscript:client:playMusic", function(data)
	local booth = ""
	...
	local p = promise.new()
	QBCore.Functions.TriggerCallback('epic-djscript:songInfo', function(cb) p:resolve(cb) end)
	...
	musicMenu[#musicMenu+1] = {
		...
		event = "epic-djscript:client:musicMenu",	args = { zoneNum = data.zoneNum }, -- ox_lib
		params = { event = "epic-djscript:client:musicMenu", args = { zoneNum = data.zoneNum } }
	}
	if previousSongs[booth] then
		musicMenu[#musicMenu+1] = {
			...
			event = "epic-djscript:client:history", args = { history = previousSongs[booth], zoneNum = data.zoneNum }, -- ox_lib
			params = { event = "epic-djscript:client:history", args = { history = previousSongs[booth], zoneNum = data.zoneNum } }
		}
	end
	if exports["xsound"]:soundExists(booth) then
		if exports["xsound"]:isPlaying(booth) then
			musicMenu[#musicMenu+1] = {
				...
				serverEvent = "epic-djscript:server:PauseResume", args = { zoneNum = data.zoneNum },
				params = { isServer = true, event = "epic-djscript:server:PauseResume", args = { zoneNum = data.zoneNum } }
			}
		...
		musicMenu[#musicMenu+1] = {
			...
			event = "epic-djscript:client:changeVolume", args = { zoneNum = data.zoneNum,  },
			params = { event = "epic-djscript:client:changeVolume", args = { zoneNum = data.zoneNum } }
		}
		musicMenu[#musicMenu+1] = {
			icon = "fas fa-stop",
			header = Loc[Config.Lan].menu["stop"],
			title = Loc[Config.Lan].menu["stop"],
			serverEvent = "epic-djscript:server:stopMusic", args = { zoneNum = data.zoneNum }, -- ox_lib
			params = { isServer = true,	event = "epic-djscript:server:stopMusic",	args = { zoneNum = data.zoneNum } }
		}
	end
...
RegisterNetEvent("epic-djscript:client:history", function(data)
	local musicMenu = {}
	if Config.Menu == "qb" then
	...
	musicMenu[#musicMenu+1] = {
		...
		event = "epic-djscript:client:playMusic", args = { job = data.job, zone = data.zoneNum },
		params = { event = "epic-djscript:client:playMusic", args = { job = data.job, zone = data.zoneNum } }
	}
	for i = #data.history, 1, -1 do
		musicMenu[#musicMenu+1] = {
			...
			event = "epic-djscript:client:historyPlay", args = { song = data.history[i], zoneNum = data.zoneNum },
			params = { event = "epic-djscript:client:historyPlay", args = { song = data.history[i], zoneNum = data.zoneNum } }
		}
	end
	if Config.Menu == "ox" then
...
RegisterNetEvent('epic-djscript:client:historyPlay', function(data) TriggerServerEvent('epic-djscript:server:playMusic', data.song, data.zoneNum) end)

RegisterNetEvent('epic-djscript:client:musicMenu', function(data)
	local dialog = nil
	if Config.Menu == "ox" then
    ...
    if dialog then
        ...
        TriggerServerEvent('epic-djscript:server:playMusic', (dialog.song or dialog[1]), data.zoneNum)
    end
end)

RegisterNetEvent('epic-djscript:client:changeVolume', function(data)
	local dialog = nil
	if Config.Menu == "ox" then
    ...
    if dialog then
        ...
        TriggerServerEvent('epic-djscript:server:changeVolume', dialog.volume, data.zoneNum)
    end
end)
